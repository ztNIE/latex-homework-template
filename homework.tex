...
\chead{\hmwkCode: \hmwkClass (\hmwkTitle)}
\rhead{\firstxmark}
\lfoot{\lastxmark}
\cfoot{\thepage}
...
\newcommand{\hmwkFinishDate}{\today}
\newcommand{\hmwkClass}{Advanced Network Technologies}
\newcommand{\hmwkCode}{ELEC5517}
\newcommand{\hmwkClassInstructor}{Prof. Bao Wei}
\newcommand{\hmwkAuthorName}{\textbf{Zhengtong Nie}}
...
\title{
    \vspace{2in}
    \textmd{\textbf{\hmwkCode: \hmwkClass}}\\
	\textmd{\textbf{\hmwkTitle}}\\
    \normalsize\vspace{0.1in}\small{\hmwkFinishDate}\\
...
\begin{homeworkProblem}
	Calculate all possible values of $(p_1, p_2, p_3)$ when Nash Equilibrium is reached.\\

    \textbf{Solution}

	First let's draw tables that could show the utility of each user in different strategies.
	...
	\begin{figure}[H]
		...
		\subfloat[User3 on] {
			\begin{tabular}{ |c|c|c| } 
			 ...
			 & User2 on & User2 off \\ 
			 \hline
				User1 on & $-5,-5,-5$ & $-5,0,-5$ \\ 
				\hline
				User1 off & $0,-5,-5$ & $0,0,10$ \\ 
				\hline
			\end{tabular}
		}
		\quad
	...
	\end{center}
	\caption{Utility of User1,2,3 in different scenerio}
	\end{figure}
	Let $U_i(on) = $ Utility of the User i get when it's on and $U_i(off) = $ Utility of the User i get when it's off\\
	Firstly, let's look at $U_1$. Because whatever strategies User2 or User3 choose, $U_1(off) = 0$, therefore,
	\begin{equation*}
		U_1 = p_1U_1(on)
	\end{equation*}

	Use the data in Table 1, we have
	\begin{align*}
		U_1(on) &= (-5p_2p_3 - 5(1-p_2)p_3 - 5(1-p_3)p_2 + 10(1-p_2)(1-p_3))\\
				&= p_1(10 - 15p_3 - 15p_2 + 15p_2p_3)
	\end{align*}
	Because N.E. requires $U_1$ would not change by toggling $p_i$ under given $p_2, p_3$, therefore, we would have
	\begin{align*}
		& 10 - 15p_3 - 15p_2 + 15p_2p_3 = 0 \; or \; p_1 = 0
	\end{align*}
	Because we are finding mixed strategies, therefore $\forall i, p_i \notin \{0, 1\}$, we have
	\begin{equation*}
		10 - 15p_3 - 15p_2 + 15p_2p_3 = 0 \tag{1}
	\end{equation*}
	The same applies to User2 and User3, we have
	\begin{align*}
		10 - 15p_3 - 15p_1 + 15p_1p_3 &= 0 \tag{2}\\ 
		10 - 15p_2 - 15p_1 + 15p_1p_2 &= 0 \tag{3}
	\end{align*}
	Use $(1)-(2)$ we have,
	\begin{align*}
		15p_1 - 15p_2 + 15(p_2p_3-p_1p_3) &= 0 \\
		(p_1-p_2)(1-p_3) &=0
	\end{align*}
	Because $p_3\neq 1$, we have
	\begin{equation}
		p_1 = p_2 \tag{4}
	\end{equation}
	Similarly, we could get $p_2 = p_3$. Let $p_1 = p_2 = p_3 = p$ and use this result in $(1)$, we have 
	\begin{align*}
		3p^2 - 6p + 2 &= 0 \\
		p &= 1 \pm \frac{\sqrt{3}}{3}
	\end{align*}

	Because $p \in (0,1)$, $p = 1 - \frac{\sqrt{3}}{3}$. $(p_1, p_2, p_3) = (1-\frac{\sqrt{3}}{3}, 1-\frac{\sqrt{3}}{3}, 1-\frac{\sqrt{3}}{3})$ is the only mixed N.E.
